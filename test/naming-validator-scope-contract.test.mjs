...
test('--scope=docs includes doc/**, docs/**, and root README.md only from root conventional docs set', () => {
  ...
  assert.equal(
    docsPaths.some((pathname) => pathname.startsWith('src/')),
    false,
  );
  assert.equal(
    docsPaths.some((pathname) => pathname === 'package.json'),
    false,
  );
  assert.equal(
...
test('--scope=app includes src/test and excludes docs, validator, and root tooling files', () => {
  ...
  assert.equal(
    appPaths.some((pathname) => pathname.startsWith('doc/')),
    false,
  );
  assert.equal(
    appPaths.some((pathname) => pathname.startsWith('docs/')),
    false,
  );
  assert.equal(
...
test('--scope=system includes root tooling files only and excludes all folders', () => {
  ...
  assert.equal(
    systemPaths.some((pathname) => pathname.startsWith('test/')),
    false,
  );
  assert.equal(
    systemPaths.some((pathname) => pathname.startsWith('doc/')),
    false,
  );
  assert.equal(
...
test('CLI file target filters to one in-scope file and reports normalized targets', () => {
  const appPaths = collectRepositoryPaths(process.cwd(), { scope: 'app' });
  const targetPath =
    appPaths.find((pathname) => pathname.endsWith('.tsx') || pathname.endsWith('.ts')) ??
    appPaths[0];
...
test('CLI directory target filters recursively within scope', () => {
  ...
  const expectedPaths = appPaths.filter(
    (pathname) => pathname === targetDirectory || pathname.startsWith(`${targetDirectory}/`),
  );
  assert.ok(
    expectedPaths.length > 0,
    'Expected src directory to contain app-scope reportable files',
  );
