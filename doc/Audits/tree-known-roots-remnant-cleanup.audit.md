...
- `doc/ValidatorSpecs/tree-known-roots-*.md`
...
|---|---|---|
| `tree/src/tree-structure-advisor.logic.mjs` `details.knownRoots` for `TREE_UNEXPECTED_TOP_LEVEL_FOLDER` | live report output / report detail | Renamed to `details.allowedTopLevelDirectories` so active report details match Tree repo-shape policy ownership. Finding code, severity, classification, message, summary behavior, and exit behavior remain stable. |
| `tree/test/tree-structure-advisor.test.mjs` assertions for `details.knownRoots` | obsolete test expectation | Updated to assert `details.allowedTopLevelDirectories` and added a focused guard that `knownRoots` is no longer emitted. |
| `tree/src/tree-occurrence-classification.logic.mjs` `isKnownTopRoot` output flag | live runtime behavior | Renamed to `isRepoShapeAllowedTopLevelDirectory` because the active predicate is repo-shape allow-list membership, not legacy known-roots authority. |
...
| `tree/src/registries/_builtin/surface-structural-home-perspective.registry.json` `current-known-root-aligned` relationship values | active docs/data reference | Renamed to `current-repo-shape-policy-aligned`; rationale wording now points at current repo-shape policy instead of known roots. |
| `doc/ValidatorSpecs/tree-structure-advisor-validator.spec.md` and `doc/ValidatorSpecs/nl-config/cfg-treeStructureAdvisor.md` active runtime boundary notes | active docs/spec reference | Updated active report-detail field and occurrence flag names. Retained explicit retirement statements for `knownTopLevelDirectories` and `topRoots[].kind` because they clarify not current runtime truth. |
| `doc/ConventionRoutines/ValidatorLoaderConverterRuntimeOwnership-Contract.md` Tree loader list | dead documentation reference | Replaced the retired `tree-known-roots-registry.logic.mjs` pointer with the current `tree-repo-shape-policy-registry.logic.mjs` pointer. |
| `doc/ConventionRoutines/registry-expansion-candidates.audit.md` and `post-registry-reassessment.audit.md` completed extraction/planning notes | historical/retired planning record with stale active wording | Reworded concise references to current repo-shape policy so these planning records no longer look like active known-roots runtime evidence. |
| `doc/ValidatorSpecs/tree-known-roots-*.md` | historical/retired planning record | Kept in place because these files are already marked as historical/retired context after issue #572. Their active-runtime-sounding statements are governed by that status banner and remain not current runtime truth. |
| `doc/ValidatorSpecs/tree-owned/**` known-roots references | historical/retired planning record or navigation record | Kept when already status-marked as historical context or navigation/supporting context; no active runtime authority was taken from these occurrences. |
| `doc/Audits/naming-tree-suite-core-usage-alignment.audit.md` known-roots retirement mentions | historical/retired planning record | Kept as issue #599 audit context. |
| `test/validator-config-strictness.test.mjs` `tmp-config-unknown-root-key.json` | unrelated false-positive occurrence | Kept; this is an “unknown root key” config strictness fixture, not Tree known-roots terminology. |
| `package.json` | no occurrence found | No change; package scripts and bins remain untouched. |
| `src/core/validator-registry.knowledge.mjs` | no occurrence found | No change; suite registry metadata remains untouched. |
