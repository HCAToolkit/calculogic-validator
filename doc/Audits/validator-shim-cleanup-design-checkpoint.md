...
4. `src/tree-structure-advisor.host.mjs` → `src/tree/tree-structure-advisor.host.mjs`
5. `src/tree-structure-advisor.logic.mjs` → `src/tree/tree-structure-advisor.logic.mjs`
...
- Canonical tree host ownership boundary is already under `src/tree/tree-structure-advisor.host.mjs`.
...
- clarifies that canonical owned paths remain `src/core/**` and `src/tree/**`
...
4. It keeps extraction paths obvious: canonical implementations remain in `src/core/**` and `src/tree/**`, and temporary legacy bridges are tightly scoped.

Policy statement (post-checkpoint target):

- `src/core/**` and `src/tree/**` are canonical_source internal module ownership paths.
...
4. **Documentation alignment**
   ...
   - keep public boundary carveouts unchanged (`src/index.mjs`, `src/naming/naming-validator.host.mjs`, `src/tree/tree-structure-advisor.host.mjs`)
