...
- Scope profiles registry (`scope-profiles.registry.json`) via `validator-scopes.logic.mjs`.
- Default validator scope policy via `DEFAULT_VALIDATOR_SCOPE` in `validator-scopes.logic.mjs` (canonical suite-owned default consumed by runtime + naming CLI/wiring).
- Special cases registry (`special-cases.registry.json`) via `naming-special-case-rules-registry.logic.mjs`.
- Exit policy registry (`exit-policy.registry.json`) via `validator-exit-policy.logic.mjs` + `validator-exit-code.logic.mjs`.
- Tree repo-shape policy registry (`repo-shape-policy.registry.json`) via `tree-repo-shape-policy-registry.logic.mjs`.
...
| Default scope fallback (CLI) | `naming/src/cli/naming-cli-args.logic.mjs` and `src/core/validator-scopes.logic.mjs` | CLI parser now initializes from suite-owned `DEFAULT_VALIDATOR_SCOPE`; runtime normalizes from same owner. | Canonical owner removes CLI/runtime drift risk. | `DEFAULT_VALIDATOR_SCOPE` in suite runtime. | `completed` | Preserve behavior (`repo`) while keeping one owner. | Parser behavior unchanged; ownership consolidated. |
...
| Exit behavior mapping | `src/registries/_builtin/exit-policy.registry.json` + `src/registries/validator-exit-policy.logic.mjs` + `src/core/validator-exit-code.logic.mjs` | Runtime now resolves ordered exit-policy predicates from bounded builtin registry and evaluates those predicates deterministically in code. | Severity/classification + strict-mode to exit-code mapping is policy contract, while semantic evaluation remains engine-owned. | `exit-policy.registry.json` (ordered predicates). | `completed` | Keep schema bounded to current semantics and preserve deterministic first-match ordering. | Extraction completed in Slice A without exit-semantic changes. |
| Tree top-level repo-shape vocabulary | `tree/src/registries/_builtin/repo-shape-policy.registry.json` + `tree/src/tree-structure-advisor.logic.mjs` | Runtime now consumes `allowedTopLevelDirectories` from bounded Tree repo-shape policy payload. | Allowed top-level directories are repository policy, not algorithmic necessity. | `repo-shape-policy.registry.json`. | `completed` | Keep payload bounded to current top-level vocabulary and preserve deterministic ordering in findings details. | Post-known-roots policy ownership is current runtime truth. |
...
- System-scope wildcard expansion plumbing in `validator-scopes.logic.mjs` while root file compatibility behavior remains coupled to core runtime knowledge.
