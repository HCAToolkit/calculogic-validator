...
- **Defer to (if conflict):**
  - `doc/ConventionRoutines/ValidatorSuite-Contracts-And-Modes.md` (suite canonical contract)
  - `doc/ValidatorSpecs/tree-structure-advisor-validator.spec.md` (tree canonical slice spec)
  - `doc/ConventionRoutines/NamingValidatorSpec.md` (naming canonical slice spec when naming boundaries are implicated)
...
- `doc/ValidatorSpecs/tree-structure-advisor-validator.spec.md` stays at the `ValidatorSpecs` root as the canonical tree slice entrypoint,
- tree-specific supporting specs and routing metadata live under `doc/ValidatorSpecs/tree-owned/`,
- the validator-owned tree NL/config note remains under `doc/ValidatorSpecs/nl-config/`.
...
5. `doc/ValidatorSpecs/tree-owned/tree-documentation-map-and-reorg-inventory.md` (this doc; navigation/index metadata under `tree-owned/`)
...
|---|---|---|---|---|---|
| `doc/ConventionRoutines/ValidatorSuite-Contracts-And-Modes.md` | Suite contract (shared validator vocabulary/modes/scope boundary) | **Canonical contract** | Runtime implementation, architecture/modeling, Codex task context | **Stay put** at current path | **Keep** |
| `doc/ValidatorSpecs/tree-structure-advisor-validator.spec.md` | Tree slice runtime/spec authority with bounded modeling notes | **Canonical slice spec** | Runtime implementation, architecture/modeling, Codex task context | **Stay put** at current path (already under `ValidatorSpecs`, implementation-facing) | **Keep** |
| `doc/ConventionRoutines/NamingValidatorSpec.md` | Naming slice runtime/spec authority consumed by tree for naming-signal boundary interpretation | **Canonical slice spec** (cross-slice supporting authority for tree tasks) | Runtime implementation, architecture/modeling, Codex task context, supporting context only for tree | **Stay put** at current path | **Reference only** for tree tasks |
| `doc/ValidatorSpecs/nl-config/cfg-treeStructureAdvisor.md` | Tree NL/config implementation note | **Supporting implementation guidance** | Runtime implementation, Codex task context, historical/reference context for implementation sequencing | **Now colocated** under `ValidatorSpecs/nl-config` with pointer stub retained at the prior repo-root NL path | **Keep** (validator-owned supporting implementation guidance) |
| `doc/ValidatorSpecs/tree-owned/validator-tree-structural-reality.audit.md` | Tree structural-reality audit baseline for advisor-signal hardening | **Audit** | Architecture/modeling, advisory-noise hardening calibration, Codex task setup context | **Now colocated** under `ValidatorSpecs/tree-owned` for tree-owned discoverability | **Keep** |
| `doc/ValidatorSpecs/tree-owned/validator-tree-report-noise-hardening.audit.md` | Tree report-vs-baseline comparison audit for advisory precision hardening targets | **Audit** | Architecture/modeling, advisory precision calibration, implementation-slice planning context | **Now colocated** under `ValidatorSpecs/tree-owned` for tree-owned discoverability | **Keep** |
| `doc/ValidatorSpecs/tree-owned/tree-addressed-occurrence-registry-source-audit.md` | Tree addressed-occurrence registry-source audit documenting verified built-in registry surfaces, runtime consumers, ownership boundaries, gaps, and the next narrow registry-backed seam for issue #668 | **Audit** | Architecture/modeling, addressed-occurrence adapter planning, Codex task setup context | **Stay put** under `ValidatorSpecs/tree-owned` for tree-owned discoverability | **Keep** |
| `doc/ConventionRoutines/FileNamingMasterList-V1_1.md` | Naming grammar/taxonomy authority consumed by tree for filename/role vocabulary boundaries | **Canonical contract** (cross-slice naming authority) | Architecture/modeling, Codex task context, supporting context only for tree | **Stay put** at current path | **Reference only** for tree tasks |
| `doc/ConventionRoutines/DeterministicStructuralAddressingSpec-Draft.md` | Bounded draft reference for report-only tree-address grammar alignment | **Draft** (bounded modeling guidance) | Architecture/modeling, supporting context only, historical/deferred decision reference | **Stay put** at current path | **Reference only** |
| `doc/ValidatorSpecs/tree-known-roots-retirement-registry-alignment.spec.md` | Known-roots retirement registry-alignment blocker spec (Slice 3) covering required-before-replacement decisions, Tree-local/shared ownership boundary contract, `test` / `tests` literal handling, structural-homes/folder-kinds replacement relationship, semantic-home + Naming bridge staging, and compatibility-adapter retirement timing | **Supporting implementation guidance** (required reading before known-roots runtime replacement work) | Runtime replacement planning, architecture/modeling, Codex task setup context for retirement sequencing | **Stay put** at current path under `ValidatorSpecs` so replacement slices can discover it before runtime changes | **Keep** (docs/spec-only planning; does not implement runtime changes, does not change registry payloads, and does not remove known-roots) |

| `doc/ValidatorSpecs/tree-known-roots-retirement-addressed-occurrence-contract.spec.md` | Known-roots retirement addressed occurrence evidence contract (Slice 4) defining address-evidence ownership boundaries and current-vs-target addressed evidence shape before runtime dependency replacement | **Supporting implementation guidance** (required reading before known-roots runtime replacement work) | Runtime replacement planning, architecture/modeling, Codex task setup context for known-roots retirement sequencing | **Stay put** at current path under `ValidatorSpecs` so replacement slices can consume the evidence contract prior to runtime migration | **Keep** (docs/spec-only contract; does not implement runtime changes, does not change registry payloads, and does not remove known-roots) |
| `doc/ValidatorSpecs/tree-known-roots-retirement-semantic-home-evidence-contract.spec.md` | Known-roots retirement semantic-home evidence contract (Slice 7) defining current semantic-home evidence reality, missing Naming-prepared inputs, and staged Tree semantic-home evidence target shape before runtime behavior migration | **Supporting implementation guidance** (required reading before known-roots semantic replacement work) | Runtime replacement planning, architecture/modeling, Codex task setup context for semantic-home evidence staging | **Stay put** at current path under `ValidatorSpecs` so replacement slices can consume semantic-home evidence staging constraints before runtime migration | **Keep** (docs/spec-only contract; does not implement runtime changes, does not change registry payloads, and does not remove known-roots) |
| `doc/ValidatorSpecs/tree-known-roots-retirement-naming-semantic-bridge-contract.spec.md` | Known-roots retirement Naming-prepared semantic evidence bridge contract (Slice 8) defining Naming-owned semantic evidence payload boundaries, Tree consumption/linkage expectations, and non-goal guardrails before Tree semantic-home evidence runtime preparation | **Supporting implementation guidance** (required reading before Tree semantic-home evidence preparation implementation work) | Runtime replacement planning, architecture/modeling, Codex task setup context for Naming→Tree semantic evidence bridge staging | **Stay put** at current path under `ValidatorSpecs` so replacement slices can consume the bridge contract before runtime migration | **Keep** (docs/spec-only bridge contract; does not implement runtime changes, does not change registry payloads, and does not remove known-roots) |
...
1. `doc/nl-config/cfg-treeStructureAdvisor.md` -> `doc/ValidatorSpecs/nl-config/cfg-treeStructureAdvisor.md`
...
2. Tree-owned supporting specs and inventories now live under `doc/ValidatorSpecs/tree-owned/`:
