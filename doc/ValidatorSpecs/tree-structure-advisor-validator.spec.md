...
For the initial route, Tree compares the Naming-owned semantic qualifier with the nearest addressed ancestor folder context from the occurrence lineage. Matching context yields `semantic-qualified-structural-container-aligned`; a different established context yields `semantic-qualified-structural-container-semantic-context-mismatch`; missing context yields `semantic-qualified-structural-container-context-unresolved`; and an unregistered embedded structural token yields `structural-token-not-registered-for-this-perspective`. Relationship outcomes remain internal prepared evidence and do not change finding codes, counts, severity, suppression, confidence policy, exit behavior, or report-envelope schema.
