...
- When the prepared replacement route is ready, folders classified as repo-shape allowed do not emit `TREE_UNEXPECTED_TOP_LEVEL_FOLDER`; repo-top folders classified as unexpected still emit the existing finding with unchanged code, severity, classification, ruleRef, path style, and details shape.
